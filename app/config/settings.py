...
class BaseAppSettings(BaseSettings, ABC):
    ...
    def communication_providers_list(self) -> List[str]:
        ...
        if isinstance(self.communication_providers, str):
            return [provider.strip() for provider in self.communication_providers.split(',')]
    ...
    def trip_reminder_intervals_list(self) -> List[int]:
        ...
        if isinstance(self.trip_reminder_intervals, str):
            return [int(interval.strip()) for interval in self.trip_reminder_intervals.split(',')]
...
class DevelopmentSettings(BaseAppSettings):
    ...
    test_phone_numbers: str = Field(
        default="", 
        validation_alias="TEST_PHONE_NUMBERS"
    )
...
class TestSettings(BaseAppSettings):
    ...
    database_url: str = "postgresql+asyncpg://postgres:test@localhost:5432/protectogram_test"
...
class ProductionSettings(BaseAppSettings):
    ...
    webhook_secret: str = Field(..., validation_alias="WEBHOOK_SECRET")  # Required in prod
