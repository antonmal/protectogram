...
def normalize_url(raw):
    ...
    if host.endswith(".internal") or host.endswith(".flycast") or host.startswith("fdaa:"):
        q["ssl"] = "disable"
        ca = {}
    else:
        q["ssl"] = "require"
...
async def test():
    try:
        ...
        async with eng.begin() as conn:
            result = await conn.execute(sa.text("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name"))
            tables = [row[0] for row in result.fetchall()]
            print('Tables in database:')
            for table in tables:
                print(f'  - {table}')
            
            result = await conn.execute(sa.text("SELECT table_name FROM information_schema.tables WHERE table_name = 'alembic_version'"))
